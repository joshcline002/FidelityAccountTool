def get_consolidated_data_filtered_down(cleaned_list_of_fidelity_data):
    consolidate_and_filter_columns = {}
    account_name_number_list = list(set(d['AccountNameNumber'] for d in cleaned_list_of_fidelity_data for val in d))
    ...
    for fidelity_data in cleaned_list_of_fidelity_data:
        ...
        consolidate_and_filter_columns[list(current_price.keys())[0]] = current_price.get(list(current_price.keys())[0])
    output = []
...
def aggregate_quantity_and_create_key(consolidate_and_filter_columns, current_account_number, current_symbol,
                                      fidelity_data):
    current_quantity = consolidate_and_filter_columns.get(f'{current_account_number}_{current_symbol}_Quantity', 0.0)
...
def aggregate_value_and_create_key(consolidate_and_filter_columns, current_account_number, current_symbol,
                                   fidelity_data):
